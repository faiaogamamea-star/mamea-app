...
const FINNHUB_KEY = 'd89nqdhr01qhi7rt9mg0';

const COLORS = {
  bg: '#04060D', surface: '#0B0F1A', card: '#111827', border: '#1F2937',
  accent: '#00C896', accentDim: '#00C89620', gold: '#F59E0B', red: '#EF4444',
  blue: '#3B82F6', text: '#F9FAFB', muted: '#6B7280', sub: '#9CA3AF',
};

const styles = {
  app: { fontFamily: "'Syne', sans-serif", background: COLORS.bg, minHeight: '100vh', color: COLORS.text, maxWidth: 480, margin: '0 auto', overflowX: 'hidden' },
  disclaimer: { background: '#1a1000', borderBottom: `1px solid ${COLORS.gold}40`, padding: '8px 16px', fontSize: 11, color: COLORS.gold, textAlign: 'center', letterSpacing: 0.3 },
  header: { padding: '16px 20px 12px', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: COLORS.surface, position: 'sticky', top: 0, zIndex: 50 },
  logo: { fontSize: 22, fontWeight: 800, letterSpacing: -0.5, color: COLORS.accent },
  logoBadge: { fontSize: 9, fontWeight: 600, background: COLORS.accentDim, color: COLORS.accent, padding: '2px 7px', borderRadius: 4, marginLeft: 8, letterSpacing: 1 },
  nav: { display: 'flex', background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, overflowX: 'auto', scrollbarWidth: 'none' },
  navItem: (active) => ({ flex: '0 0 auto', padding: '12px 16px', fontSize: 12, fontWeight: active ? 700 : 500, color: active ? COLORS.accent : COLORS.muted, borderBottom: active ? `2px solid ${COLORS.accent}` : '2px solid transparent', cursor: 'pointer', whiteSpace: 'nowrap', letterSpacing: 0.5, transition: 'all 0.15s' }),
  content: { padding: '16px', paddingBottom: 80 },
  card: { background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 16, marginBottom: 12 },
  sectionTitle: { fontSize: 11, fontWeight: 700, color: COLORS.muted, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12, marginTop: 4 },
  assetRow: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: `1px solid ${COLORS.border}` },
  assetIcon: (color) => ({ width: 36, height: 36, borderRadius: 10, background: color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, marginRight: 12, flexShrink: 0 }),
  assetName: { fontSize: 14, fontWeight: 700, color: COLORS.text },
  assetSub: { fontSize: 11, color: COLORS.muted, marginTop: 2, fontFamily: "'DM Mono', monospace" },
  price: { fontSize: 14, fontWeight: 700, fontFamily: "'DM Mono', monospace", textAlign: 'right' },
  change: (pos) => ({ fontSize: 11, fontWeight: 600, color: pos ? COLORS.accent : COLORS.red, textAlign: 'right', marginTop: 2, fontFamily: "'DM Mono', monospace" }),
  pill: (color) => ({ display: 'inline-block', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 20, background: color + '20', color, letterSpacing: 0.5 }),
  signalCard: { background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 14, marginBottom: 10, borderLeft: `3px solid ${COLORS.accent}` },
  planCard: (featured) => ({ background: featured ? COLORS.accent + '12' : COLORS.card, border: `1px solid ${featured ? COLORS.accent : COLORS.border}`, borderRadius: 14, padding: 20, marginBottom: 14 }),
  btn: (variant) => ({ display: 'block', width: '100%', padding: '12px', borderRadius: 10, border: variant === 'outline' ? `1px solid ${COLORS.accent}` : 'none', background: variant === 'solid' ? COLORS.accent : 'transparent', color: variant === 'solid' ? '#000' : COLORS.accent, fontFamily: "'Syne', sans-serif", fontSize: 13, fontWeight: 700, cursor: 'pointer', letterSpacing: 0.5, marginTop: 14, textAlign: 'center' }),
};
...
const STOCKS_META = [
  { id: 'nvda', name: 'NVIDIA',    symbol: 'NVDA', icon: '⬡', color: '#76B900', sector: 'Tech' },
  { id: 'aapl', name: 'Apple',     symbol: 'AAPL', icon: '', color: '#818CF8', sector: 'Tech' },
  { id: 'tsla', name: 'Tesla',     symbol: 'TSLA', icon: 'T', color: '#EF4444', sector: 'EV' },
  { id: 'msft', name: 'Microsoft', symbol: 'MSFT', icon: '⊞', color: '#00BCF2', sector: 'Tech' },
  { id: 'amzn', name: 'Amazon',    symbol: 'AMZN', icon: 'a', color: '#FF9900', sector: 'Retail' },
];
...
function AssetRow({ asset }) {
  ...
  const noData = asset.price === null || asset.price === undefined;
  return (
    <div style={styles.assetRow}>
      ...
      <div>
        <div style={{ ...styles.price, color: noData ? COLORS.muted : COLORS.text }}>
          {fmtPrice(asset.price, asset.symbol)}
        </div>
        {!noData && asset.change !== null
          ? <div style={styles.change(pos)}>{pos ? '+' : ''}{Number(asset.change).toFixed(2)}%</div>
          : <div style={{ fontSize: 10, color: COLORS.muted, textAlign: 'right' }}>—</div>
        }
      </div>
    </div>
  );
...
function Dashboard({ cryptoData, stockData, cryptoLoading, stockLoading, lastUpdated }) {
    ...
    <div style={styles.content}>
      <div style={styles.card}>
        <div style={styles.sectionTitle}>
          Market Overview
          {lastUpdated && <span style={{ fontSize: 10, color: COLORS.accent, marginLeft: 8, fontWeight: 400, letterSpacing: 0 }}>· Live Data</span>}
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {[
            { label: 'BTC Price', val: cryptoLoading ? 'Loading...' : fmtPrice(btc?.price, 'BTC'), color: COLORS.gold },
            { label: 'ETH Price', val: cryptoLoading ? 'Loading...' : fmtPrice(eth?.price, 'ETH'), color: '#818CF8' },
            { label: 'NVDA',      val: stockLoading  ? 'Loading...' : fmtPrice(nvda?.price, 'NVDA'), color: '#76B900' },
          ].map(m => (
            <div key={m.label} style={{ flex: '1 1 30%', background: COLORS.surface, borderRadius: 8, padding: '10px 12px' }}>
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: m.color, fontFamily: "'DM Mono', monospace" }}>{m.val}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={styles.card}>
        <div style={styles.sectionTitle}>Top Crypto · Live via CoinGecko</div>
        {cryptoLoading
          ? <div style={{ color: COLORS.muted, fontSize: 13, padding: '12px 0' }}>Fetching live prices...</div>
          : cryptoData.slice(0, 3).map(a => <AssetRow key={a.id} asset={a} />)
        }
      </div>

      <div style={styles.card}>
        <div style={styles.sectionTitle}>Top Stocks · Live via Finnhub</div>
        {stockLoading
          ? <div style={{ color: COLORS.muted, fontSize: 13, padding: '12px 0' }}>Fetching live prices...</div>
          : stockData.slice(0, 3).map(a => <AssetRow key={a.id} asset={a} />)
        }
      </div>
...
function Markets({ cryptoData, stockData, cryptoLoading, stockLoading }) {
  const [sub, setSub] = useState('crypto');
  const data = sub === 'crypto' ? cryptoData : sub === 'stocks' ? stockData : REALESTATE;
  const loading = sub === 'crypto' ? cryptoLoading : sub === 'stocks' ? stockLoading : false;
  const source = sub === 'crypto' ? '· Live via CoinGecko' : sub === 'stocks' ? '· Live via Finnhub' : '· Reference data';
    ...
    <div style={styles.content}>
      ...
      <div style={styles.card}>
        <div style={styles.sectionTitle}>
          ...
          <span style={{ fontSize: 10, color: COLORS.accent, marginLeft: 8, fontWeight: 400, letterSpacing: 0 }}>{source}</span>
        </div>
        {loading
          ? <div style={{ color: COLORS.muted, fontSize: 13, padding: '12px 0' }}>Fetching live prices...</div>
    ...
    <div style={styles.content}>
      <div style={{ ...styles.card, borderColor: COLORS.accent + '40', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, color: COLORS.sub, lineHeight: 1.6 }}>
          Automated observations based on publicly available market data. <strong style={{ color: COLORS.text }}>Not financial advice.</strong>
        </div>
      </div>
  ...
  return (
    <div style={styles.content}>
      <div style={{ ...styles.card, borderColor: COLORS.gold + '40', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, color: COLORS.sub, lineHeight: 1.6 }}>
          Publicly shared portfolio allocations for research purposes. Past returns are hypothetical and do not guarantee future performance.
        </div>
      </div>
      ...
      {PORTFOLIOS.map((p, i) => (
        <div key={i} style={styles.card}>
          ...
          <div style={{ fontSize: 11, color: COLORS.sub, background: COLORS.surface, padding: '8px 10px', borderRadius: 8, lineHeight: 1.5 }}>{p.allocation}</div>
...
function Plans() {
  const plans = [
    { name: 'Free',  price: '$0',  period: 'forever',   featured: false, features: ['Live crypto prices via CoinGecko', 'Live stock prices via Finnhub', 'Basic market signals', 'Top news headlines', 'Market overview dashboard'] },
  ...
  return (
    <div style={styles.content}>
      ...
      <div style={{ fontSize: 11, color: COLORS.muted, textAlign: 'center', lineHeight: 1.6, padding: '8px 0 16px' }}>
        ...
        Not a registered investment advisor. Not a broker-dealer.
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const loadCrypto = useCallback(async () => {
    try {
      ...
      setCryptoData(CRYPTO_META.map(m => ({ ...m, price: data[m.geckoId]?.usd ?? null, change: data[m.geckoId]?.usd_24h_change ?? null })));
  ...
  const loadStocks = useCallback(async () => {
    try {
      const results = await Promise.all(STOCKS_META.map(async s => {
        const d = await fetchStockPrice(s.symbol);
        return { ...s, price: d.price, change: d.change };
      }));
  ...
  useEffect(() => {
    loadCrypto();
    loadStocks();
    const c = setInterval(loadCrypto, 60000);
    const s = setInterval(loadStocks, 60000);
  ...
  const renderTab = () => {
    switch (activeTab) {
      case 'Dashboard':  return <Dashboard cryptoData={cryptoData} stockData={stockData} cryptoLoading={cryptoLoading} stockLoading={stockLoading} lastUpdated={lastUpdated} />;
      case 'Markets':    return <Markets cryptoData={cryptoData} stockData={stockData} cryptoLoading={cryptoLoading} stockLoading={stockLoading} />;
      case 'Signals':    return <Signals />;
      case 'News':       return <NewsTab />;
      case 'Portfolios': return <TopPortfolios />;
      case 'Plans':      return <Plans />;
      default:           return <Dashboard cryptoData={cryptoData} stockData={stockData} cryptoLoading={cryptoLoading} stockLoading={stockLoading} lastUpdated={lastUpdated} />;
    }
  };
    ...
    <div style={styles.app}>
      ...
      <div style={styles.header}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {lastUpdated && <span style={{ fontSize: 10, color: COLORS.accent, fontFamily: "'DM Mono', monospace" }}>● LIVE</span>}
      ...
      <div style={styles.nav}>
        {TABS.map(tab => (
          <div key={tab} style={styles.navItem(activeTab === tab)} onClick={() => setActiveTab(tab)}>{tab.toUpperCase()}</div>
        ))}
      </div>
      {renderTab()}
      <div style={{ padding: '24px 20px', borderTop: `1px solid ${COLORS.border}` }}>
        <div style={{ fontSize: 10, color: COLORS.muted, textAlign: 'center', lineHeight: 1.8 }}>
          ...
          Crypto: <a href="https://coingecko.com" target="_blank" rel="noopener noreferrer" style={{ color: COLORS.accent, textDecoration: 'none' }}>CoinGecko</a> · Stocks: <a href="https://finnhub.io" target="_blank" rel="noopener noreferrer" style={{ color: COLORS.accent, textDecoration: 'none' }}>Finnhub</a><br />
